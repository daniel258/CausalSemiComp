...
####################################################################################
EMcausalSC <- function(data, Xnames, max.iter = 10000, w = NULL, eps.conv = 0.0001, init.thetas = c(1, 1))
{
  n.sample <- nrow(data)
  ...
  XmatA0 <- as.matrix(dplyr::select(data.predictA0, Xnames))
  XmatA1 <- as.matrix(dplyr::select(data.predictA1, Xnames))
  XmatA0T12 <- as.matrix(dplyr::select(data.predictA0T12, Xnames))
  XmatA1T12 <- as.matrix(dplyr::select(data.predictA1T12, Xnames))
