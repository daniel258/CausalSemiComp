WrapCausalSCboot <- function(data, i = i, all.times, Ltime, out.times)
{
  ...
  res <- CausalSC(T1 = data$T1[i], T2 = data$T2[i],
                  ...
                  all.times = all.times)
  ind.times <- all.times[all.times %in% out.times]
  res.out <- c(res$etaA0, res$etaA1, res$S2A0.all.times[ind.times], res$S2A0.all.times[ind.times],
               res$S2A0.all.times[ind.times], res$S2A1.all.times[ind.times],
